import asyncio,os,inspect,logging as lg,functools
    ...
    async def __call__(self,request):
        ...
        if kw is None:
            ...
            for k,v in request.match_info.items():
                if k in kw:
                    lg.warning('Duplicate arg name in named arg and kw args: %s' % k)
                kw[k] = v
        ...
        lg.info('call with args %s' % str(kw))
        try:
...
def add_router(app,fn):
    ...
    lg.info('add route %s %s => %s(%s)' % (method, path, fn.__name__, ', '.join(inspect.signature(fn).parameters.keys())))
