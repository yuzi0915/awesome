...
async def create_pool(loop,**kw):
    ...
    __pool = await aiomysql.create_pool(
        host = kw.get('host','192.168.181.128'),
...
async def select(sql,args,size =None):
    ...
    async with __pool.get() as conn:
        async with conn.curor(aiomysql.DictCursor) as cur:
            await cur.execute(sql.repace('?','%s'),args or ())
            if size:
...
async def execute(sql,args,autommit=True):
    log(sql)
    async with __pool.get() as conn:
        if not autommit:
            await conn.begin()
    ...
    def __new__(cls,name,bases,attrs):
        ...
        for k,v in attrs.items():
            ...
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    if primaryKey:
                        raise AttributeError('Dulicate primary key for field:%s'%k)
                    primaryKey = k
                else:
                    fields.append(k)
        if not primaryKey:
